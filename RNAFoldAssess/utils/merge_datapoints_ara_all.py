...
for ch in ["1", "2", "3", "4", "5", "Mt", "Pt"]:
    exon_locs_raw = {}
    ch_coord_loc = chromosome_map[f"chromosome_{ch}"]["coords"]
    ch_seq_loc = chromosome_map[f"chromosome_{ch}"]["seq"]
    ch_json_loc = chromosome_map[f"chromosome_{ch}"]["json"]
    with open(ch_coord_loc) as cf:
        coord_data = cf.readlines()

    with open(ch_seq_loc) as sf:
    ...
    with open(ch_json_loc) as jf:
        datapoints = json.load(jf)
    
    f = open(f"/common/yesselmanlab/ewhiting/data/rasp_data/loc_info_chromosome_{ch}.txt", "w")
    print(f"Working {ch}")
    counter = 0
    len_data = len(exon_locs_raw)
    for exon, locs in exon_locs_raw.items():
        try:
            locs = [int(i) for i in locs]
            counter += 1
            if counter % 250 == 0:
                print(f"Working {counter} of {len_data} in {ch}. Current exon: {exon}")
            f.write(f"{exon}\n")
            for dp in datapoints:
                if all(i in locs for i in dp["coordinates"]):
                    f.write(f"{dp['name']}\n")
        except Exception as e:
            print(f"Problem in Chromosome {ch}, exon {exon}: {e}")
