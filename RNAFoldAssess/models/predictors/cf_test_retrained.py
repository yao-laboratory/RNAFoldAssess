...
class ContraTestRetrained:
    def __init__(self):
        self.output = ""

    def execute(self, contra_path, seq_file, remove_file_when_done=False):
        exec_string = f"{contra_path}/src/contrafold predict {seq_file} --params /work/yesselmanlab/ewhiting/data/bprna/retrain_test/optimize.params.iter40"
    ...
    def get_ss_prediction(self):
        if self.output == "":
            raise Exception(f"ContraTestRetrained exception: no output generated")
