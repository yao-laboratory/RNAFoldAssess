...
class MXFold2RetrainedYData:
    def __init__(self):
        self.output = ""
        self.path_to_model = "/work/yesselmanlab/ewhiting/chem_map_to_bpseq/retrainings/retrain_mxfold2/model.conf"
    ...
    def execute(self, fasta_file, path="", remove_file_when_done=False):
        ...
        if remove_file_when_done:
            try:
                ...
                print(f"MXFold2RetrainedYData: Couldn't find {fasta_file} to delete")

    def get_ss_prediction(self):
        if self.output == "":
            raise Exception(f"MXFold2RetrainedYData exception: no output generated")
