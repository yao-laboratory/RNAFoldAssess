...
class MXFold2RetrainedRNAndria:
    def __init__(self):
        self.output = ""
        base_dir = "/work/yesselmanlab/ewhiting/chem_map_to_bpseq/retrainings"
        self.path_to_model = f"{base_dir}/retrain_mxfold2/rnandria/model_all.conf"
    ...
    def execute(self, fasta_file, path="", remove_file_when_done=False):
        ...
        if remove_file_when_done:
            try:
                ...
                print(f"MXFold2RetrainedRNAndria: Couldn't find {fasta_file} to delete")

    def get_ss_prediction(self):
        if self.output == "":
            raise Exception(f"MXFold2RetrainedRNAndria exception: no output generated")
