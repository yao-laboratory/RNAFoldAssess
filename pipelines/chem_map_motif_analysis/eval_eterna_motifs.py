...
print("Loading data ...")
with open("/mnt/nrdstor/yesselmanlab/ewhiting/reports/higher_level_analysis/latest/all_predictions/eterna_motif_locs_and_reactivities.json") as fh:
    data = json.load(fh)
...
for dp in data:
    ...
    for m in models:
        ...
        for motif in motifs:
            acc, p, percent_of_positions = info_about_score(dp, motif, m, chemical_mapping_method)
            try:
...
with open(f"{dest_dir}/eterna_motif_data.json", "w") as fh:
