...
mxfold = MXFold()
mxfold_path = os.path.abspath("/home/yesselmanlab/ewhiting/mxfold/build/mxfold")
...
f = open("/common/yesselmanlab/ewhiting/reports/mxfold_dms_pipeline_report.txt", "w")
f.write(f"{headers}\n")
...
for dp in data_points:
    ...
    mxfold.execute(mxfold_path, input_file_path)
    prediction = mxfold.get_ss_prediction()
    try:
        score = DSCI.score(
            dp.sequence,
            prediction,
        ...
        f.write(f"MXFold, {dp.name}, {accuracy}, {p}, DMS\n")
        counter += 1
...
# about dataset
f2 = open("/common/yesselmanlab/ewhiting/reports/about_pipeline.txt", "w")
