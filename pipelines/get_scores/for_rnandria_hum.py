...
pred_dir = "/mnt/nrdstor/yesselmanlab/ewhiting/reports/rnandria/canonical"

dps = set()
for m in models:
    with open(f"{pred_dir}/{m}_human_mRNA_predictions.txt") as fh:
        lines = fh.readlines()
    
    lines = [line.split(", ") for line in lines]
    accs = [line[4] for line in lines]
    with open(f"scores/rnandria_scores/{m}_ribonanza_human_mRNA_accs.txt", "w") as fh:
        fh.write(",".join(accs))
...
for m, accs in model_map.items():
    fstring = ",".join(accs)
    with open(f"scores/rnandria_scores/{m}_rnandria_human_mRNA_ordered_accs.txt", "w") as fh:
        fh.write(fstring)
