...
dps = set()
for m in models:
    with open(f"{pred_dir}/{m}_pri_miRNA_predictions.txt") as fh:
        lines = fh.readlines()
    
    lines = [line.split(", ") for line in lines]
    accs = [line[4] for line in lines]
    with open(f"scores/rnandria_scores/{m}_ribonanza_pri_miRNA_accs.txt", "w") as fh:
        fh.write(",".join(accs))
...
for m, accs in model_map.items():
    fstring = ",".join(accs)
    with open(f"scores/rnandria_scores/{m}_rnandria_pri_miRNA_ordered_accs.txt", "w") as fh:
        fh.write(fstring)
