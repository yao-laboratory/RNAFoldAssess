...
pred_dir = "/mnt/nrdstor/yesselmanlab/ewhiting/reports/ydata/canonize"

dps = set()

for m in models:
    with open(f"{pred_dir}/{m}_YesselmanDMS_report.txt") as fh:
        lines = fh.readlines()
    
    lines = [line.split(", ") for line in lines]
    accs = [line[4] for line in lines]
    with open(f"scores/ydata_scores/{m}_ydata_accs.txt", "w") as fh:
        fh.write(",".join(accs))
...
for m, accs in model_map.items():
    fstring = ",".join(accs)
    with open(f"scores/ydata_scores/{m}_ydata_ordered_accs.txt", "w") as fh:
        fh.write(fstring)
