...
## How to Use the Built-in Scorers of This Project

The RNAFoldAssess framework comes with three scorers already built in. Scorers are algorithms that take a predicted structure and some ground-truth information about the "real" secondary structure, and then assign a score between 0 and 1 to that prediction. A score of zero means the prediction was not accurate at all, a score of 1 means the prediction was as accurate as it could've been. The prediction algorithm's performance is generally evaluated by how high its predictions scored for a given dataset.

Out of the box, RNAFoldAssess has three scorers for two different *kinds* of scoring: one for datasets where the ground truth data is from chemical mapping experiments, and two for datasets where the ground truth is provided as a secondary structure in dot-bracket notation.

### The DSCI scorer

The `DSCI` class is for evaluating secondary structure when the ground-truth data is from chemical mapping experiments (typically SHAPE or DMS). In such datasets, the true secondary structure is inferred from the reactivities rather than known explicitly. The DSCI method makes use of the Mann-Whitney U-test to come up with an accuracy score and p-value, both between 0 and 1.

If you have an RNA, it's chemical mapping reactivities, and a predicted secondary structure, you can use the static `score` method to get the accuracy and p-value of the Mann-Whitney U-test. The scoring algorithm is slightly different depending on whether the chemical mapping data is DMS or SHAPE, so you have to specify which kind of chemical mapping data the reactivities are from.
...
```
If this is DMS data, you call the DSCI scoring method like so:

```python
from RNAFoldAssess.models.scorers import DSCI

score = DSCI.score(sequence, predicted_structure, reactivities, DMS=True)
```

If you run this, your `score` variable will be a Python dictionary with values similar to:
...
If this data is SHAPE data, you call the DSCI scoring method like so:

```python
from RNAFoldAssess.models.scorers import DSCI

score = DSCI.score(sequence, predicted_structure, reactivities, SHAPE=True)
```
...
This method will raise an exception if the sequence, secondary structure prediction, and reactivity data are not the same length. Note that this method uses the `mannwhitneyu` method from the `scipy.stats` module. Sometimes, the reactivity data will make this method behave weirdly. In these cases, the method raises a `ValueError`, in which case the `DSCI` class will raise a custom `DSCIValueError`. If this is happening to you, try to unnegate and/or normalize your reactivity data.

### How to Use the Base-Pair Scorers

The `BasePairScorer` class is for evaluating a secondary structure prediction when your ground truth data is in dot-bracket notation. This scorer does not have a static scoring method, you have to instantiate it in order to get thoe most out of it.

To use the `BasePairScorer` class, you have to instantiate it with a `true_structure` and `predicted_structure` constructor argument. The `true_structure` is your ground truth secondary structure in dot-bracket notation, and the `predicted_structure` is a secondary structure prediction generated by some prediction tool, also in dot-bracket notation.

Once you've instantiated the object, the constructor will automatically calculate the true positives, false positives, and false negatives of the secondary structure prediction. You can then call the object's `evaluate` method to calculate the sensitivity, positive predictive value, and F1 score of the prediction.

**NOTE** If we call the count of true positives TP, the count of false positives FP, and the count of false negatives FN, then
* Sensitivity = TP / (TP + FN)
* Positive predictive value (PPV): TP / (TP + FP)
* F1 Score: (2 * sensitivity * PPV) / (sensitivity * PPV)

Consider the following example: Suppose you have an RNA molecule and you know the secondary structure is (((...((...))...))). You run the molecule's sequence through a prediction algorithm and it produces the prediction "(.(...((...))...).)"

To evaluate this prediction, you would run the following code:
...
known = "(((...((...))...)))"
predicted = "(.(...((...))...).)"
bps = BasePairScorer(known, predicted)
bps.evaluate()

bps.sensitivity # 0.8
bps.ppv # 1.0
bps.f1 # 0.888888888888889
```

The class also has a convenient `parse_structure` method that takes in a dot-bracket notation and returns a list of two-tuples. The two-tuples represent coordinates of base-paired nucleotides. For example, the following code:

```python
from RNAFoldAssess.models.scorers import BasePairScorer

ss = "..((((...))))..((..))."
coords = BasePairScorer.parse_structure(ss)
```
...
Additionally, there is a `BasePairPseudoknotScorer` scoring class. This class is used the same as the `BasePairScorer` class, but handles predictions that use square brackets to annotate the location of pseudoknots in a secondary structure. Not all secondary structure prediction algorithms will predict pseudoknots, so there is a dedicated scorer for that use-case.
